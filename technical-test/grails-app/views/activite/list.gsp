			...
			<table>
				<thead>
					<tr>
						...
						<th><g:message code="activite.moniteur.label" default="Moniteur" /></th>
					
						<g:sortableColumn property="nomActivite" title="${message(code: 'activite.nomActivite.label', default: 'Nom Activite')}" />
				...
				<g:each in="${activiteInstanceList}" status="i" var="activiteInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${activiteInstance.id}">${fieldValue(bean: activiteInstance, field: "description")}</g:link></td>
					
						<td>${fieldValue(bean: activiteInstance, field: "moniteur")}</td>
					
						<td>${fieldValue(bean: activiteInstance, field: "nomActivite")}</td>
